...
export default function makeProgram(program) {
  program
    ...
    .option('--no-color', 'Output plain text without color') // @ts-expect-error
    .action(action);
}
...
async function action(args, opts, command) {
  ...
  // --exclude
  config.exclude = opts.exclude // @ts-expect-error
 ...
 *
 * @param {{quiet:boolean,recursive:boolean,exclude:RegExp[]}} config options
 ...
 *
 * @param {{recursive:boolean,exclude:RegExp[]}} config options
 * @param {string} dir input directory
 ...
 *
 * @param {{datauri:DataUri,quiet:boolean}} config
 * @param {string} file
 * @param {string} output
 * @return {Promise<unknown>}
 */
function optimizeFile(config, file, output) {
  return fs.promises.readFile(file, 'utf8').then(
    (data) => processSVGData(config, { path: file }, data, output, file), // @ts-expect-error
 ...
 *
 * @param {{datauri:DataUri,quiet:boolean}} config
 * @param {{file?:string,path?:string}|null} info
 ...
 *
 * @param {{quiet:boolean,recursive:boolean,exclude:RegExp[]}} config
 * @param {string} input
 * @param {string} output
 * @param {{code:string,path:string}} error
 * @return {Promise<unknown>}
 */
